...
window.onload = function() {
    ...
    const setupFrames = () => {
        ...
        section1.style.height = (imgHeight + 10) + 'px';
            ...
            frame.onclick = function() {
                if (!isExperienceSelected) {
                    ...
                    setTimeout(() => {
                        ...
                        cardNameDisplay.textContent = `${cardNames[randomCardIndex]} ${orientation}`;
    ...
    viewMeaningButton.onclick = function() {
        // Hiện section-mean chỉ khi khung hình S1 không phải là 0.jpg
        const khungHinhS1 = document.querySelector('.khung-hinh-s1');
        if (khungHinhS1.style.backgroundImage !== `url(images/0.jpg)`) {
            sectionMean.style.display = 'block';
            viewMeaningButton.classList.add('active');

            // Gửi thông tin đã tổng hợp tới ChatGPT
            const cardName = cardNameDisplay.textContent.split(' ')[0]; // Lấy tên lá bài
            sendToGPT(`Ý nghĩa của lá bài ${cardName} là gì?`); // Gửi yêu cầu đến ChatGPT
        } else {
            showNotification("Bạn chưa chọn lá bài nào."); // Thông báo khi chưa chọn lá bài
        }
    };

    // Hàm gửi thông điệp đến API ChatGPT
    function sendToGPT(message) {
        fetch('/api/chat', { // Đảm bảo endpoint phù hợp với server của bạn
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
